...
export class DateUtilsService {
  ...
  public static unixTimeToDate(unixTime: number) {
    return new Date(new Date(unixTime).toLocaleDateString("en-US",{timeZone:"America/Sao_Paulo"}));
  }
  ...
  public static plusDays(date: Date, numberDays: number): Date {
    ...
    let datePlused: Date = new Date(new Date(date.setDate(date.getDate() + numberDays)).toLocaleDateString("en-US",{timeZone:"America/Sao_Paulo"}));
    return datePlused
  }

  public static setHours(date: Date, hour: number): Date {
    return new Date(new Date(date.setHours(hour, 0, 0, 0)).toLocaleDateString("en-US",{timeZone:"America/Sao_Paulo"}));
  }
